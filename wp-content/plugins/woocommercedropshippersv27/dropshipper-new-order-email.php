						...
						foreach ($real_products[$user_login] as $item) {
							...
							if( (!isset($options['show_prices'])) || ( isset($options['show_prices']) && ($options['show_prices'] == 'Yes') ) ){
								if($options['text_string'] == "Yes"){ //show prices
									echo '<td style="text-align: left; vertical-align: middle; border: 1px  solid  #eee;"><span class="amount">'. woocommerce_price(( ((float) $item['line_total']) + ((float) $item['line_tax']) )).'<br>(Your earning: '.woocommerce_price((float) ($drop_price*($item['qty']-$item['redunfed_items'] )) ).')</span></td></tr>';
								}
								else{
									echo '<td style="text-align: left; vertical-align: middle; border: 1px  solid  #eee;"><span class="amount">Your earning: '.woocommerce_price((float) ($drop_price*($item['qty']-$item['redunfed_items'])) ).'</span></td></tr>';
								}
							}
							...
							if($options['send_pdf'] == 'Yes'){
								...
								$pdf->MultiCell(100,4, __($my_item_post->post_title) . ' (SKU: '.$SKU.")\n". ((!empty($item_meta))?(strip_tags($item_meta->display( true, true ))."\n"):'') );
								$pdf->Cell(10, 2, ' ');
								$pdf->Ln();
						...
						?>
							...
							<?php
								if($options['text_string'] == "Yes"){ //show full prices
									echo woocommerce_price($drop_subtotal) . '<br>(Your earning: '.woocommerce_price((float) $drop_total_earnings_no_shipping ).')';
								}
								else{
									echo 'Your earning: '.woocommerce_price((float) $drop_total_earnings_no_shipping );
								}
							?>
							</span></td>
							</tr>
									...
									if( (!isset($options['show_prices'])) || ( isset($options['show_prices']) && ($options['show_prices'] == 'Yes') ) ){
										...
										?>
											<tr><th style="text-align: left; border: 1px  solid  #eee;" colspan="2"><?php echo __('Dropshipper Shipping Fee:','woocommerce-dropshippers'); ?></th>
												<td style="text-align: left; border: 1px  solid  #eee;"><?php echo woocommerce_price((float) $nat); ?></td>
											</tr>
										<?php
										}
										...
										?>
											<tr><th style="text-align: left; border: 1px  solid  #eee;" colspan="2"><?php echo __('Dropshipper Shipping Fee:','woocommerce-dropshippers'); ?></th>
												<td style="text-align: left; border: 1px  solid  #eee;"><?php echo woocommerce_price((float) $inter); ?></td>
											</tr>
										<?php	
										}
									}
							...
							<tr><th style="text-align: left; border: 1px  solid  #eee;" colspan="2"><?php echo __('Order Total:','woocommerce-dropshippers'); ?></th>
								<td style="text-align: left; border: 1px  solid  #eee;"><span class="amount"><?php
									if($options['can_see_email_shipping'] == 'Yes'){
										if($options['text_string'] == "Yes"){ //show full prices
											echo woocommerce_price( (float) ($drop_subtotal_plus_dropshipper_shipping + $order->get_total_shipping()));
											echo '<br>(Your earning: ' . woocommerce_price( (float) $drop_total_earnings ) . ')';
										}
										else{
											echo 'Your earning: ' . woocommerce_price( (float) $drop_total_earnings );
										}

									}
									else{
										if($options['text_string'] == "Yes"){ //show full prices
											echo woocommerce_price( (float) $drop_subtotal_plus_dropshipper_shipping);
											echo '<br>(Your earning: ' . woocommerce_price( (float) $drop_total_earnings_no_shipping ) .')';
										}
										else{
											echo 'Your earning: ' . woocommerce_price( (float) $drop_total_earnings_no_shipping );
										}
